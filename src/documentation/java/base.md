...
- 缺点：
  ...
  - 安全问题：让我们能动态作改变类的属性同时也增加了类的安全隐患

### 29.Java中的动态代理是什么？有哪些应用？
代理可以分为`静态代理`和`动态代理`,动态代理又分为`JDK的动态代理`和`CGLIB动态代理`
代理类在程序运行时创建的代理方式被称为动态代理，例如，spring的AOP就是动态代理

__静态代理__：代理对象和实际对象都继承了同一个接口，在代理对象中指向的是实际对象的实例，这样对外暴露的是代理对象而真正调用的是Real Object
...
- JDK的动态代理只实现了`InvocationHandler`接口，重写`invoke`方法便可完成代理的实现
- JDK的动态代理利用反射生成代理类`Proxyxx.class`代理类字节码，并生成对象
- JDK动态代理之所以只能代理接口是因为代理类本身已经extends了Proxy，而Java是不允许多继承的，允许实现多个接口
- __优点__：解决了静态代理中冗余的代理实现类问题
- __缺点__：JDK动态代理是基于接口设计实现的，如果没有接口，会抛异常

__CGLIB代理__：

- 由于JDK动态代理限制了只能基于接口设计，而对于没有接口的情况，JDK方式解决不了
- CGLIB采用了非常底层的字节码技术，其原理是通过字节码技术为一个类创建子类，并在子类中采用方法拦截的技术拦截所有父类方法的调用，顺势织入横切面逻辑，来完成动态代理的实现
- 实现`MethodInterceptor`接口，重写`intercept`方法，通过Enhancer类的回调方法来实现
- 但是CGLIB在创建代理对象时所花费的时间要比JDK多的多，所以对于单例的对象，因为无需频繁创建对象，用CGLIB合适，反之，使用JDK方式要更为合适一些
- 同时，由于CGLIB时采用动态创建子类的方法，对于final方法，无法进行代理
- __优点__：没有接口也能实现动态代理，而且采用字节码增强技术，性能不错
- __缺点__：技术实现相对难理解
...
2. super可以用来访问父类的成员方法或者变量，当子类成员变量或方法与父类有相同的名字时也会覆盖父类的成员变量或方法，这个时候要想访问父类的成员变量或方法只能通过super关键字来访问，子类方法中的super.方法名()不要需要位于第一行

### 32.字节和字符的区别？

字节(byte)是计量单位，表示数据量多少，是计算机信息技术用于计量存储内容的一种计量单位，通常情况下一字节等于八位
...
### 37.final修饰StringBuffer后还可以append吗？

StringBuffer理解为缓冲区，不能通过赋值符号对其进行赋值，使用append进行值修改，没有创建新对象

对其加以final修饰，final修饰引用对象，代表引用对象不可变，StringBuffer实现append()没有产生新的对象，所以不会报错

### 38.Java中IO的分类

按处理数据类型：字符流(文本文件，只能处理纯文本)，字节流(二进制，可以处理一切文件)

按流向：输入流(Reader,inputStream),输出流(Writer,outputStream)
